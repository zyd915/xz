...
class VideolistModule
{
    ...
    {
        $articles = VideolistModel::model()
            ->getDb()
            ->limit(5)
            ->order('sorder desc, aid desc')
            ->where(array('isnew' => VideolistModel::IS_NEW_YES))
            ->queryAll();
    ...
    {
        // 文章分类
        $articelsCats = VideoCatModel::model()
            ->getDb()
            ->order('sorder desc')
            ->limit(3)
            ->queryAll();
   ...
   public function substr_cut($str, $len, $charset="utf-8"){
    ...
    public function getCgal($limit,$cut = 'false',$arr = array()){
        ...
        $cgal = VideolistModel::model()->getDb()->where(array('cid'=>$getcgal))->limit($limit)->order('aid desc')->queryAll();
        if($cut == 'true'){
    ...
    public function getCatArticles($catName, $count)
    {
        if (is_numeric($catName)) :
            ...
            $cid = VideoCatModel::model()->getDb()
                ->where(array('name' => $catName))
                ->queryColumn('cid');
        endif;
        if ($cid) :
            $articles = VideolistModel::model()->getDb()->limit($count)->where(array('cid' => $cid))->queryAll();
    ...
    public function statusToggle($aid)
    {
        ...
        $status = VideolistModel::model()->getDb()
            ->where($condtion)
            ->queryColumn('status');

        // 状态调换
        if ($status == VideolistModel::STATUS_APPROVED) :
            $status = VideolistModel::STATUS_FORBIDDEN;
        else :
            $status = VideolistModel::STATUS_APPROVED;
        endif;
        ...
        VideolistModel::model()->getDb()->where($condtion)->update($updateStatus);
    ...
    public function getArticlesDetailInfo(array $articles)
    {
        ...
        if (arComp('validator.validator')->checkMutiArray($articles)) :
            ...
            $article = $articles;
            $article['catName'] = VideoCatModel::model()
                ->getDb()
    ...
    {
        // 文章分类
        $cats = VideoCatModel::model()->getDb()
            ->order('sorder desc')
            ->queryAll();
