...
$(".ad-toggle").click(function() {
    ...
    var obj = $(this);
    // ajax 请求
    $.get(JSV.PATH_SERVER + 'admin/Ad/statusToggle', {id: $(this).val()}, function(data) {
        if (data.ret_msg == '0') {
...
$(".ad-edit").click(function () {
    ...
    location.href = JSV.PATH_APP_SERVER + 'Ad/adManager?opt=edit&aid=' + aid;
});
...
jQuery(document).ready(function() {
    $('#upload_ad_logo').uploadify({
        ...
        //swf的相对路径，必写项
        uploader: JSV.PATH_APP_SERVER + 'Ad/uploadAdPic?sess=' + JSV.SESSION,
        //服务器端脚本文件路径，必写项
        uploadLimit: 10,
        ...
        onUploadSuccess: function(file,data,res) {
            ...
            $("[name='galleryid']").attr('value', obj[0]['gid']);
...
function delImg(gid, oid, obj)
{
    layer.confirm('确定要删除此图片', function () {
        var lid = layer.load('...');
        $.post(JSV.PATH_APP_SERVER + 'Ad/delImg', {gid: gid, oid: oid}, function (res) {
            layer.close(lid);
            if (res.success == "1") {
                gallery('remove', gid);
                $(obj).parent().remove();
...
function delAd(aid)
{
    layer.confirm('确定要删除此产品', function () {
        var lid = layer.load('...');
        $.post(JSV.PATH_APP_SERVER + 'Ad/AdManager/opt/delete', {aid: aid}, function (res) {
            layer.close(lid);
            if (res.success == "1") {
                layer.msg(res.ret_msg);
                location.reload();
...
$("td").mouseover(function () {
    var imgStr = '<img src="'+$(this).parent().attr('imgurl')+'">';
    imgUrlId = layer.tips(imgStr, this, {guide: 2, style:['background-color:#ffffff', 'width:auto']});
}).mouseout(function () {
    layer.close(imgUrlId);
});


// 点击
$('#ad-add-cate').bind('click', myHandlerFunction);
var first = true;
function myHandlerFunction(e) {
    if (first) {
        layer.tips($('#tips-cate-show').html(), this, {guide:2,style:['background:#ffffff']});
    } else {
        layer.closeAll();
    }
